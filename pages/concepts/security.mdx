...
The screenshot below defines the row/column-level policy. This policy expects the `state` parameter in the SQL expression. It allows access to all columns except `discount`.

`state in {{ state }}`

![rcls](pages/concepts/security_images/rcls.png)

After authenticating the user, you can pass in value depending on the user's profile.
