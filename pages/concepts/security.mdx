...
Here is how it works: When a user is authenticated by your app, based on the user's tenancy and profile, you pass in the appropriate `parameter` along with your `dashboard id` and `secret`. Semaphor then uses the `connection-level policy` and the `parameter`, and generates an auth token that can only be used to access the data authorized to that user.
...
Now you can apply this policy to your dashboard. When you authenticate the user, based on their tenacy and profile, you can consturct a connection policy object as shown below. Pass this along with the dashboard ID and secret to the token API. Remember to perform this step on the server side.
...
Here's how it works: When a user is authenticated by your app, depending on their tenancy and profile, you pass the appropriate `parameter` for that policy, along with your `dashboard id` and `secret`. Semaphor then uses the `row-level policy` and the `parameter` to generate a `token` that can only be used to access data authorized for that user.

The screenshot below defines the row/column-level policy. This policy expects the `state` parameter in the SQL expression. It allows access to all columns except the `discount` column.
...
The SQL expresssion will be expanded as

```sql

SELECT * FROM sales_data WHERE state in ('California', 'Nevada', 'Washington')
