...
If you want do visualize two cohors side by side for a set of categories, you can create a pyramid chart using the following query structure.

```sql filename="Card SQL"
SELECT category, ship_mode, COUNT(*)
FROM sales_data
WHERE ship_mode in ('Standard Class', 'Second Class') -- cohort
