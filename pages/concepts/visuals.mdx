...
To create a stacked combo chart with dual axis, your query must be structured as shown below.
...
**Step 1:**
In this example, we will plot `L1`, `L2`, `L3` columns as lines, and `B1`, `B2`, `B3` columns as stacked bars. But first, we will plot all columns as a `bar` chart and then customize it to meet our requirements.
...
**Step 3:**
And finally, need to stack the the bars by setting `stacked` property to `true` for both `x` `(options.scales.x)` and `y` `(options.scales.y)` axes in the chart config.
