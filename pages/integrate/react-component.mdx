...
<Callout type="warning" emoji="⚠️" title="React 18">
  Currently, the Semaphor component is only compatible with React 18. We are
...
Next, you will need to get the `filterId` from Semaphor console. You can get the `filterId` from the filter's settings (⚙️) popover, and then hover on the ID icon to see the ID.

![Filter ID](pages/integrate/images/filter_id.png)

Now in the `DashboardContainer` component, you can use the hooks to get the current state of the filter and to update the filter.
